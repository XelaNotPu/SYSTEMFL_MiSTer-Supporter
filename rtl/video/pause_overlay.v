...
// pause_overlay.v
//
// Pause-overlay module for the ZN-1 MiSTer core (fleet renderer), System FL variant:
// 36-column band (the whole 288-pixel FL line), 64-row text grid, ROMs in MLABs (the FL
// core has no block RAM left) initialised from rtl/pause_assets/*.mif, and TEXT_LOOP /
// SCROLL_WRAP generated into rtl/pause_assets/pause_params.vh by tools/gen_pause_assets.py
// from pause_src/pause.txt (sha256-tracked).
//
...
module pause_overlay (
   ...
   // ----- Asset sizes -----
   localparam [11:0] TEXT_WIDTH   = 12'd288;   // 36 cols x 8 px = 288 px text band (the whole FL line)
   localparam [11:0] TEXT_HEIGHT  = 12'd1024;  // 64 lines x 16 px
   ...
   `include "rtl/pause_assets/pause_params.vh"   // TEXT_LOOP / SCROLL_WRAP from pause.txt
   ...
   wire  [5:0] text_col   = text_x_off[8:3];   // col within the line (0..35)
   ...
   // ===========================================================================
   wire [7:0]  text_char;
   wire [11:0] text_addr = (text_line * 7'd36) + text_col;   // 64*36 = 2304 < 4096
   ...
   altsyncram #(
      ...
      .ram_block_type("MLAB"),
      .init_file("rtl/pause_assets/patreon_text.mif"),
      .lpm_type("altsyncram")
   ) text_rom (
      .clock0(clk),
      .address_a(text_addr),
   ...
   altsyncram #(
      ...
      .outdata_reg_a("UNREGISTERED"),
      .ram_block_type("MLAB"),
      .init_file("rtl/pause_assets/patreon_color.mif"),
      .lpm_type("altsyncram")
   ) colour_rom (
      .clock0(clk),
      .address_a(text_line[5:0]),
      .q_a(line_colour_idx)
   );
   ...
   altsyncram #(
      ...
      .outdata_reg_a("UNREGISTERED"),
      .ram_block_type("MLAB"),
      .init_file("rtl/pause_assets/font_8x16.mif"),
      .lpm_type("altsyncram")
   ) font_rom (
      .clock0(clk),
      .address_a(font_addr),
