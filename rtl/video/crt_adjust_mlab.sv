...
module crt_adjust_mlab #(
    ...
    parameter RAMSTYLE  = "no_rw_check, MLAB"  // SUPER22: "no_rw_check, MLAB" on the System 22 core
)
(
    input              clk,
    ...
    // SUPER22 (2026-09-13): AW 10 -> 11. The upstream buffer holds 512 samples per bank
    // ({bank, wrp[AW-2:0]}); System 22 lines are 814 pixels (640 active, 25.6 MHz), so a
    // 512-deep bank wrapped. 1024 per bank = 2048 x 24 bits.
    localparam integer AW = 11;  // 2048 samples (two 1024 ping-pong banks)
    ...
    // ------------------------------------------------------------------
    // SUPER22: RAMSTYLE parameter -- the System 22 core uses every M10K on the device
    // (553/553), so its copy of the buffer lives in MLABs (~77 MLAB = ~770 ALM).
    // SUPER22 MLAB variant (System 22 core, every M10K taken): a 2048-deep MLAB array costs ~3k ALM
    // (64 blocks + 64:1 read muxes) and did NOT fit (build s22pat2: 42,010/41,910). Store only the
    // DEPTH active pixels of each line, indexed from the active start, at 18-bit RGB666 (the 2 LSBs
    // per channel are dropped while CRT Adjust is On; the Off path is native 24-bit) -> 40 MLABs.
    localparam integer DEPTH = 640;
    (* ramstyle = "no_rw_check, MLAB" *) reg [17:0] mem [0:2*DEPTH-1];
    integer ii;
    initial for (ii = 0; ii < 2*DEPTH; ii = ii + 1) mem[ii] = 18'd0;
    reg [9:0] wa;                 // write index relative to the active start
    initial wa = 0;
    ...
    wire [9:0] wa_now = (lhb & ~lhb_l) ? 10'd0 : wa;   // first active pixel -> index 0
    always @(posedge clk) if (pxl_cen) begin
        lhb_l <= lhb;
        if (lhb && wa_now < DEPTH) mem[bank ? wa_now + DEPTH : wa_now] <= {r_in[7:2], g_in[7:2], b_in[7:2]};
        wa <= wa_now + 10'd1;
        if (hs_rise_in) begin
            wrp  <= {AW{1'b0}};
            hmax <= wrp;
    ...
    // ------------------------------------------------------------------
    reg [23:0] rd_data;          // combinational expansion of rd_q (RGB666 -> 888), see below
    reg        pass_q;
    initial begin
        pass_q = 0;
    end
    ...
    // SUPER22 fix (2026-09-13): the upstream ternary mixed a signed operand with the unsigned
    // {(AW+2){1'b0}} -> the whole expression became UNSIGNED and a negative hoffset was ZERO-extended
    // (-10 -> 502): every leftward H-Position read a wrapped address (sim/crtadj). Both arms signed.
    wire signed [AW+1:0] hoff_s  = (HPOS_MODE == `HPOS_CONTENTSHIFT)
                                   ? $signed({{(AW+1-8){hoffset[8]}}, hoffset})
                                   : $signed({(AW+2){1'b0}});
    ...
    // read index relative to the active start: pass_q is true exactly when 0 <= ri < hb0-hb1 (= DEPTH)
    wire signed [AW+1:0] ri_s = rdcnt_s - hb1_s - hoff_s;
    wire [9:0] rd_addr = ri_s[9:0];
    reg  [17:0] rd_q;
    initial rd_q = 0;
    always @(posedge clk) if (pxl2_cen) begin
        rd_q    <= mem[~bank ? rd_addr + DEPTH : rd_addr];
        pass_q  <= (rdcnt_s >= (hb1_s + hoff_s)) && (rdcnt_s < (hb0_s + hoff_s)) && ~vb_active;
    end
    always @* rd_data = {rd_q[17:12], rd_q[17:16], rd_q[11:6], rd_q[11:10], rd_q[5:0], rd_q[5:4]};   // RGB666 -> 888
    ...
    // ------------------------------------------------------------------
    // SUPER22 fix (2026-09-13): 10-bit tap -- a 9-bit 9'(VTOTAL) overflows for VTOTAL = 525
    // (9'(525) = 13), so every negative V-Shift indexed the wrong line; the shreg index needs 10 bits.
    wire [9:0] vshift_tap = voffset[5]
        ? (10'(VTOTAL) + {{4{voffset[5]}}, voffset})  // negativo -> VTOTAL - |N|
